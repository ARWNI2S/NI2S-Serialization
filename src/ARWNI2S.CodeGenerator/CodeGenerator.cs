...
using System.Text;
using static ARWNI2S.CodeGenerator.SyntaxGeneration.SymbolExtensions;
...
namespace ARWNI2S.CodeGenerator
{
    public class CodeGeneratorOptions
    {
        ...
        public List<string> ConstructorAttributes { get; } = new() { "ARWNI2S.NI2SConstructorAttribute", "Microsoft.Extensions.DependencyInjection.ActivatorUtilitiesConstructorAttribute" };
    ...
    {
        internal const string CodeGeneratorName = "NI2SCodeGen";
            ...
            foreach (var asm in assembliesToExamine)
            {
                foreach (var symbol in asm.GetDeclaredTypes())
                {
                    ...
                    if (FSharpUtilities.IsUnionCase(LibraryTypes, symbol, out var sumType) && ShouldGenerateSerializer(sumType))
                    {
                        ...
                        {
                            throw new NI2SGeneratorDiagnosticAnalysisException(InaccessibleSerializableTypeDiagnostic.CreateDiagnostic(sumType));
                        }
                    ...
                    else if (ShouldGenerateSerializer(symbol))
                    {
                        ...
                        {
                            throw new NI2SGeneratorDiagnosticAnalysisException(InaccessibleSerializableTypeDiagnostic.CreateDiagnostic(symbol));
                        }
                        ...
                        else
                        {
                            ...
                            {
                                throw new NI2SGeneratorDiagnosticAnalysisException(CanNotGenerateImplicitFieldIdsDiagnostic.CreateDiagnostic(symbol, fieldIdAssignmentHelper.FailureReason));
                            }
        ...
        private void ComputeAssembliesToExamine(IAssemblySymbol asm, HashSet<IAssemblySymbol> expandedAssemblies)
        {
            ...
            foreach (var attr in attrs)
            {
                ...
                if (declaringAsm is null)
                {
                    ...
                    throw new NI2SGeneratorDiagnosticAnalysisException(diagnostic);
                }
                else
                {
