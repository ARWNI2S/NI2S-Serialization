...
namespace ARWNI2S.Serialization.Internal
{
    public static class ReferencedAssemblyProvider
    {
        ...
        public static void AddFromDependencyContext(HashSet<Assembly> parts, Assembly assembly = null)
        {
            ...
            {
                if (!lib.Name.Contains("ARWNI2S.Serialization", StringComparison.Ordinal) && !lib.Dependencies.Any(dep => dep.Name.Contains("ARWNI2S.Serialization", StringComparison.Ordinal)))
                {
                    continue;
                }
